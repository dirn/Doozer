...
def test_import_application_non_doozer_app():
    ...
    with pytest.raises(CommandError) as e:
        ...
        assert (
            "app must be an instance of a Doozer application. Got "
...
@pytest.mark.parametrize(
    "arguments, a, b", (([], 1, 2), (["2"], 2, 2), (["2", "3"], 2, 3))
)
...
@pytest.mark.parametrize(
    "arguments, expected",
    ((["-q"], 1), (["-qq"], 2), (["--quiet"], 1), (["--quiet", "--quiet"], 2)),
)
...
@pytest.mark.parametrize(
    ...
    (("-q", 1, None), ("-qq", 2, None), ("-v", None, 1), ("-vv", None, 2)),
)
def test_register_commands_quiet_and_verbose(monkeypatch, arguments, quiet, verbose):
...
@pytest.mark.parametrize(
    "arguments, expected",
    ((["-v"], 1), (["-vv"], 2), (["--verbose"], 1), (["--verbose", "--verbose"], 2)),
)
